...
// pins into a container; owns pan/zoom.
export class MapView {
  ...
  focusOnProvince(name) {
    ...
    if (!bbox) return;
    const bw = Math.max(bbox.x1 - bbox.x0, 8),
      bh = Math.max(bbox.y1 - bbox.y0, 8);
    const padFactor = 0.7;
    const w = bw * (1 + padFactor),
      h = bh * (1 + padFactor);
  ...
  resetView() {
    this._setViewBox({ ...this.fullView });
  }
  ...
  render() {
    ...
    const provinces = this.extent.shapes
      .map((sh) => {
    ...
    const metro = this.metroHubs
      .map((h, i) => {
        ...
        return (
          `<circle class="metro-hub-pulse" cx="${cx.toFixed(1)}" cy="${cy.toFixed(1)}" r="${r.toFixed(1)}"/>` +
    ...
    const pins =
      !this.showFacilities || !this.facilities.length
        ? ""
        : this.facilities
            .map((f) => {
              const { x: cx, y: cy } = projectToPixel(f.lon, f.lat, this.extent);
              const px = cx - 12 * PIN_SCALE,
                py = cy - 24 * PIN_SCALE;
    ...
    this.svgRoot.innerHTML = `<svg viewBox="0 0 ${this.extent.width} ${this.extent.height}">${defs}${ocean}<g filter="url(#landShadow)">${provinces}</g>${metro}${pins}</svg>`;

    this.svgRoot.querySelectorAll(".province").forEach((el) => {
      el.addEventListener("click", () => {
        if (!this._dragMoved) this.onSelect?.(el.dataset.province);
      });
    });
  ...
  _applyViewBox() {
    ...
    if (svg)
      svg.setAttribute(
        "viewBox",
        `${this.viewBox.x.toFixed(2)} ${this.viewBox.y.toFixed(2)} ${this.viewBox.w.toFixed(2)} ${this.viewBox.h.toFixed(2)}`
      );
  }
  ...
  zoomBy(factor, cx, cy) {
    ...
    let newW = this.viewBox.w * factor,
      newH = this.viewBox.h * factor;
    const minW = this.fullView.w * 0.06,
      minH = this.fullView.h * 0.06;
    newW = Math.min(this.fullView.w, Math.max(minW, newW));
    newH = Math.min(this.fullView.h, Math.max(minH, newH));
    const ratioX = (cx - this.viewBox.x) / this.viewBox.w,
      ratioY = (cy - this.viewBox.y) / this.viewBox.h;
  ...
  _wirePanZoom() {
    ...
    this.svgRoot.addEventListener(
      ...
      (e) => {
        ...
        const cx = this.viewBox.x + relX * this.viewBox.w,
          cy = this.viewBox.y + relY * this.viewBox.h;
    ...
    const end = () => {
      isPanning = false;
      this.svgRoot.querySelector("svg")?.classList.remove("panning");
    };
